...
def chat_tool_hint() -> str:
    ...
    return (
        ...
        "**别把「重新翻译」当成「重新导出」**：用户说「重新翻译/再翻一遍/从头翻译/重跑一遍/换设置再翻」→ 必须调 "
        "**run_translate**（重新提取 + 重新翻译 + 导出，等价于从头重跑，当前界面设置——含「OCR表格重建」等导出选项"
        "——在启动时读取并生效）；`re_export` 只是拿**上一次的译文**重新生成文件（秒级、不重译），"
        "只有在用户明确说「按我刚才的修改重新导出/不用重译」时才用。用错工具会让用户以为设置没生效。\n"
