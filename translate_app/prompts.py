...
{{"glossary": {{"源词": "目标词"}}, "style": "……", "keep": [0, 3], "page_strategy": {{"3": "batch"}}, "notes": "……"}}

规则：
1. **glossary**：只收录下面「候选术语」里出现过的词（{term_line}），键必须与原文完全一致。没有把握就**不要写**——写错会把整篇的译名锁死。
2. **style**：用一句中文说明本篇的文体与约定（称谓、单位与金额写法、编号风格、人名写法等），不超过 120 字。它会被逐页翻译遵循。
3. **keep**：要**保留原文**的文本块扁平索引。数字/金额/公式块本就不翻译，不必列出；没有明确理由就不要写（保留得越多，译文越不完整）。
4. **page_strategy**：可选字段，用「页码(从 1 起) → \"batch\"」标出**普通文本页**可以只做一次批量翻译（省掉逐页复核循环；之后仍会过确定性审计，审计不过会自动改回逐页——所以这是省时间的建议，不是免检）。扫描页/图表页/待确认页**不要**写；拿不准就整个留空。
5. **notes**：给用户看的一句话说明。

拿不准的字段就留空（{{}} / 空串 / 空数组）。这不是「必须给答案」的问题——给错的代价比留空大得多。
输出要**短**：glossary 最多 8 条、style 最多 120 字、keep 最多 20 个索引、page_strategy 最多 30 个页号、notes 一句话。（长文档请只写最有把握的那几条——写满反而会被截断，等于什么都没有。）
