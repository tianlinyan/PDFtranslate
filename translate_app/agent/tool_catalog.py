...
TOOL_CATALOG: list[ToolDef] = [
    ...
    _tool("get_settings",
          ...
          "用户问“当前设置是什么”时用；**不要为了「开始翻译」先调它**——「开始翻译」直接调 run_translate，"
          "界面设置会在启动时自动读取生效。",
          {}, [], CAT_READ, target="source", audience=("chat",)),
    _tool("goto_page",
          "在预览窗口显示指定页（原文/译文侧）。",
    ...
    _tool("re_export",
          ...
          "「OCR表格重建」等导出选项以外的设置变更。**不要反问用户「要重新翻译还是重新导出」**——"
          "「重新翻译/开始翻译」语义唯一，就是全新翻译。",
    ...
    _tool("run_translate",
          ...
          "**用户说「开始翻译/再次开始翻译/重新翻译/再翻一遍」时直接调它**——「开始翻译」永远＝全新翻译"
          "（重新提取 + 重新翻译 + 导出，当前界面设置自动生效）；**不要先问确认、不要先 get_settings 汇报设置、"
          "不要问“要重新翻译还是重新导出”**。"
