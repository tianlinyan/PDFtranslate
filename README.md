...
> 当前版本：**v0.1.2**（版本号定义于 `translate_app/__init__.py` 的 `__version__`）
...
* **进度显示**：翻译过程中进度条实时显示完成百分比；提取文本阶段为不确定进度，
  翻译阶段按已翻译块数实时更新。
* **设置与预览**：主界面 **「设置」** 可编辑**术语表**（增删「源词→目标词」并保存到
  `glossary.json`）与**扫描页 OCR 开关**。翻译完成后可点 **「预览译文」** 每页按
  双语（译文 + 可折叠原文）审阅，再决定是否保存。
